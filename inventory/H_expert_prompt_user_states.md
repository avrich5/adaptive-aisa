...
9. **Заповни self_check чесно.** Якщо щось не вдалося — постав `false` і виправ перш ніж видати JSON. `any_field_skipped_or_templated: false` має бути true після перевірки.
...
11. **Можна розбити відповідь на 2-3 частини** якщо боїшся не вмістити все в один блок. Правила розбиття — в таблиці вище. Кожна частина — окремий валідний JSON з власним meta. `submission_id` однаковий для всіх частин. `part_number` і `total_parts` чесні. Якщо вирішив розбити — вкажи `total_parts` у Part 1 коректно, не міняй потім.
